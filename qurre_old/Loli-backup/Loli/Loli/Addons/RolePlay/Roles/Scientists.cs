...
namespace Loli.Addons.RolePlay.Roles
{
    internal class Scientists
    {
        ...
        internal static void SpawnRandomSafe(Player pl)
        {
            if (!Plugin.RolePlay) return;
            pl.Tag += "RPSpawnHide";
            if (pl.Role != RoleType.Scientist) pl.SetRole(RoleType.Scientist);
            Timing.CallDelayed(0.5f, () => SpawnRandom(pl));
        }
        internal static void SpawnRandom(Player pl)
        {
            pl.Tag = pl.Tag.Replace("RPSpawnHide", "");
        ...
        internal void Spawn(SpawnEvent ev)
        {
            ...
            if (ev.Player.Tag.Contains("RPSpawnHide")) return;
