	...
	static class Clear
	{
		...
		static IEnumerator<float> ClearManyItems()
		{
			for (; ; )
			{
				...
				var picks = UnityEngine.Object.FindObjectsOfType<ItemPickupBase>().Where(x => !Pickups.Contains(x.NetworkInfo.Serial));
				bool b1 = picks.Count() > 500;
				bool b2 = Map.Ragdolls.Count > 100;
				if (b1) Timing.RunCoroutine(ClearItems(picks.Select(x => Pickup.Get(x))));
