...
{
    internal static class AirDrop
    {
        private static Model PlaneModel;
        private static bool _calling = false;
        internal static void Waiting()
        {
            _calling = false;
            PlaneModel = new("AirDrope_Plane", new(450, 1031, -57), new(0, 180));
        ...
        {
            if (_calling) return;
            PlaneModel.GameObject.transform.position = new(450, 1031, -57);
            _calling = true;//180  170  160  -10  -160  -170  -180
        ...
        private static readonly List<DropItem> PossibleItems = new()
        {
        ...
        public class Drop
        {
            ...
            public class DropController : MonoBehaviour
            {
                ...
                private void OnCollisionEnter(Collision _)
                {
                    ...
                    if (_.gameObject.name.Contains("Collision Box") || _.gameObject.name.Contains("Don't call it a grave, it's the future you chose"))
                    {
                ...
                private void OnTriggerEnter(Collider other)
                {
                    ...
                    for (int i = 0; i < _itm.Amout; i++)
                        new Item(_itm.Type).Spawn(transform.position);
