...
namespace MongoDB.scp343
{
	public partial class EventHandlers343
	{
		...
		public void PocketEnter(EnteringPocketDimensionEventArgs ev)
		{
			...
			if (ev.Player.ReferenceHub.queryProcessor.PlayerId == scp343?.queryProcessor.PlayerId)
			{
				if (ev.Item.id == ItemType.Ammo9mm)
				{
					try
					{
						...
						ReferenceHub player = pList[Extensions.Random.Next(pList.Count)];
						if (player == null)
						{
				...
				if (ev.Item.id == ItemType.Painkillers)
				{
				...
				if (ev.Item.id == ItemType.SCP500)
				{
								...
								if (dead != null)
								{
									if (dead.Role == RoleType.Spectator)
									{
										if (DRole.ContainsKey(dead.ReferenceHub.queryProcessor.PlayerId))
										{
											...
											Timing.CallDelayed(0.2f, () => dead.Health = 20);
											Timing.CallDelayed(0.5f, () => dead.Position = ev.Player.Position);
											Timing.CallDelayed(0.5f, () => dead.ClearInventory());
					...
					{
						scp343.Hint(Configs.wait.Replace("{0}", $"{120 - resurrectiontime}"), 1);
						Timing.CallDelayed(1U, () => scp343.Hint(Configs.wait.Replace("{0}", $"{120 - resurrectiontime}"), 1));
						Timing.CallDelayed(2U, () => scp343.Hint(Configs.wait.Replace("{0}", $"{120 - resurrectiontime}"), 1));
						Timing.CallDelayed(3U, () => scp343.Hint(Configs.wait.Replace("{0}", $"{120 - resurrectiontime}"), 1));
						Timing.CallDelayed(4U, () => scp343.Hint(Configs.wait.Replace("{0}", $"{120 - resurrectiontime}"), 1));
					}
				}
				ev.IsAllowed = false;
			}
		...
		public void OnPickupItem(PickingUpItemEventArgs ev)
		{
			if (ev.Player.ReferenceHub.queryProcessor.PlayerId == scp343?.queryProcessor.PlayerId)
			{
				...
				{
					ev.Pickup.Delete();
					plugin.armor.jugarmor = MongoDB.Extensions.SpawnItem(ev.Pickup.itemId, ev.Pickup.durability, ev.Pickup.transform.position, ev.Pickup.transform.rotation);
				}
				else if (ev.Pickup.durability == scp035.EventHandlers.dur)
				{
					ev.IsAllowed = false;
					MongoDB.Extensions.SpawnItem(ev.Pickup.itemId, ev.Pickup.durability, ev.Pickup.transform.position, ev.Pickup.transform.rotation);
					return;
				}
				else if (ev.Pickup == scp228.EventHandlers228.vodka)
				{
					ev.IsAllowed = false;
					scp228.EventHandlers228.vodka = MongoDB.Extensions.SpawnItem(ev.Pickup.itemId, ev.Pickup.durability, ev.Pickup.transform.position, ev.Pickup.transform.rotation);
					return;
				}
				...
				{
					ev.Pickup.Delete();
					MongoDB.Extensions.SpawnItem(ItemType.Medkit, 10000, ev.Player.ReferenceHub.transform.position);
				}
				else if (ev.Pickup.ItemId == ItemType.MicroHID)
				{
					ev.Pickup.Delete();
					MongoDB.Extensions.SpawnItem(ItemType.MicroHID, 10000, ev.Player.ReferenceHub.transform.position);
				}
				else if (ev.Pickup.ItemId == ItemType.GrenadeFrag || ev.Pickup.ItemId == ItemType.GrenadeFlash)
				{
					ev.Pickup.Delete();
					MongoDB.Extensions.SpawnItem(ItemType.Adrenaline, 10000, ev.Player.ReferenceHub.transform.position);
				}
				else
				{
					ev.Pickup.Delete();
					MongoDB.Extensions.SpawnItem(ev.Pickup.ItemId, 50, ev.Pickup.transform.position, ev.Pickup.rotation,
		...
		public void SleepGood(ReferenceHub player)
		{
			...
			player.SetPosition(-229, 1993.7f, -67);
