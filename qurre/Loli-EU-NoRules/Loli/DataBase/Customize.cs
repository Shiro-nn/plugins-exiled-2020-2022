...
namespace Loli.DataBase
{
	...
	static class Customize
	{
		...
		internal static GensMod GetGens(Player pl)
		{
			...
			if (role == RoleTypeId.ClassD || role == RoleTypeId.ChaosConscript) return data.ClassD;
			if (role == RoleTypeId.Scientist || role == RoleTypeId.NtfSpecialist) return data.Scientist;
			if (role == RoleTypeId.FacilityGuard) return data.Guard;
			if (role.GetTeam() == Team.FoundationForces) return data.Mtf;
			if (role.GetTeam() == Team.ChaosInsurgency) return data.Chaos;
			return new();
		}
