...
namespace TextChat.Events
{
	public class PlayerHandler
	{
		...
		public void OnPlayerJoin(PlayerJoinEvent ev)
		{
			...
			((CharacterClassManager)ev.Player.characterClassManager).TargetConsolePrint(((Mirror.NetworkBehaviour)ev.Player.scp079PlayerScript).connectionToClient, $"Добро пожаловать в чат!", "green");
			ev.Player.Broadcast(10, "<color=lime>На этом сервере существует чат</color>\n<color=red>.help</color>", true);
		}
