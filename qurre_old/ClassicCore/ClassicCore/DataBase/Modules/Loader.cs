...
namespace ClassicCore.DataBase.Modules
{
	internal class Loader
	{
		...
		private static void AddClan(string tag)
		{
			...
			List<int> Boosts = new();
			{
				var url = $"{Init.APIUrl}/clan?tag={tag}&token={Init.ApiToken}&type=all_users";
			...
			{
				var url = $"{Init.APIUrl}/clan?tag={tag}&token={Init.ApiToken}&type=boosts";
				var request = WebRequest.Create(url);
				request.Method = "POST";
				using var webResponse = request.GetResponse();
				using var webStream = webResponse.GetResponseStream();
		...
		{
			Init.Socket.On("database.get.data", obj =>
			{
		...
		internal void LoadData(Player pl)
		{
			...
			Init.Socket.Emit("database.get.data", new object[] { pl.UserId.Replace("@steam", "").Replace("@discord", ""),
		...
		internal void LoadRoles(Player player, UserData data)
		{
			...
			Init.Socket.Emit("database.get.donate.ra", new object[] { data.login, 2, player.UserId });
			Init.Socket.Emit("database.get.donate.roles", new object[] { data.id, 2, player.UserId });
			Init.Socket.Emit("database.get.donate.customize", new object[] { data.login, player.UserId });
			if (data.trainee)
			{
		...
		{
			Init.Socket.On("ChangeFreezeSCPServer", data =>
			{
				BDDonateRoles doc = JsonConvert.DeserializeObject<BDDonateRoles>($"{data[0]}");
				if (doc.server != -1 && doc.server != 2) return;
		...
		{
			Init.Socket.On("database.get.donate.roles", obj =>
			{
				string userid = obj[1].ToString();
				var pl = Player.Get(userid);
				if (pl is null) return;
			...
			Init.Socket.On("database.get.donate.ra", obj =>
			{
				string userid = obj[1].ToString();
				var pl = Player.Get(userid);
				if (pl is null) return;
