...
namespace ClassicCore.DataBase
{
	...
	internal class Customize
	{
		...
		private void Init()
		{
			ClassicCore.Init.Socket.On("database.get.donate.customize", obj =>
			{
		...
		internal static GensMod GetGens(Player pl)
		{
			...
			if (pl.Role == RoleType.ClassD || pl.Role == RoleType.ChaosConscript) return data.ClassD;
			if (pl.Role == RoleType.Scientist || pl.Role == RoleType.NtfSpecialist) return data.Scientist;
			if (pl.Role == RoleType.FacilityGuard) return data.Guard;
			var team = pl.Team;
			if (team == Team.MTF) return data.Mtf;
			if (team == Team.CHI) return data.Chaos;
			return new();
		}
