...
namespace ClassicCore.DataBase.Modules
{
	internal class Admins
	{
		...
		internal void Prefixs(RaRequestPlayerListEvent ev)
		{
			...
			string text = "\n";
			foreach (Player pl in Player.List.OrderBy(x => x.Id))
			{
			...
			if (ev.Issuer.Nickname != "Dedicated Server")
			{
				try
				{
					if (Manager.Static.Data.Users.TryGetValue(ev.Issuer.UserId, out var _data))
						Init.Socket.Emit("database.admin.ban", new object[] { _data.id, 1 });
				}
				catch { }
				try
				{
		...
		internal void Kick(KickEvent ev)
		{
			...
			{
				if (Manager.Static.Data.Users.TryGetValue(ev.Issuer.UserId, out var _data))
					Init.Socket.Emit("database.admin.kick", new object[] { _data.id, 1 });
			}
			catch { }
			try
			{
