	...
	partial class EventHandlers
	{
		...
		public static void SpawnJG(ReferenceHub JG)//2 events for correct spawn
		{
			if (scp682 == null)
			{
				...
				Timing.CallDelayed(1.0f, () => GameCore.Console.singleton.TypeCommand($"/hp {JG.GetPlayerId()}. {Configs.health}"));
		...
		public static void Spawn343(ReferenceHub sss)//spawn scp 343
		{
			if (scp343 == null)
			{
				...
				sss.playerEffectsController.EnableEffect<CustomPlayerEffects.Scp207>();
		...
		public static void SpawnJG2(ReferenceHub JG)//spawn for replace player disconnect(682)
		{
			...
			{
				JG.characterClassManager.SetClassID(RoleType.Scp93989);
				Timing.CallDelayed(1.0f, () => GameCore.Console.singleton.TypeCommand($"/hp {JG.GetPlayerId()}. 400"));
				JG.ClearBroadcasts();
		...
		public static void selectspawnSSS()//search player for spawn 343
		{
			...
			if (true)
			{
				...
				{
					if (rand.Next(1, 101) < Configs.spawnchance343)
