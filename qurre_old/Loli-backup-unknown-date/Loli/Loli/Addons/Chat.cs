...
namespace Loli.Addons
{
    internal class Chat
    {
        internal class Message
        {
            ...
            internal Module.RoleType Role = Module.RoleType.None;
            internal Player PrivateTo;
            internal Vector3 Position;
            internal Message(string text, Player author, MessageType type, Module.RoleType role, Player privateTo, Vector3 pos)
            {
                Text = text;
                Author = author;
                Type = type;
        ...
        internal static void Console(SendingConsoleEvent ev)
        {
            ...
            if (type == "отключить" || type == "disable")
            {
            ...
            else if (type == "включить" || type == "enable")
            {
                if (Disables.Contains(ev.Player.UserId))
                {
            ...
            void Send(string _text, MessageType type, Vector3 pos, Player to = null)
            {
                ...
                {
                    Name = $"{ev.Player.Nickname} - {ev.Player.UserId}"
                };
                Embed embed = new()
                {
