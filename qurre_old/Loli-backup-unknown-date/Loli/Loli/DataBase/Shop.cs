	...
	public class Shop
	{
		...
		internal void Pickup(PickupItemEvent ev)
		{
			...
			if (ev.Pickup.Type == ItemType.Coin)
			{
				...
				ev.Pickup.Destroy();
				ev.Player.Broadcast(3, $"<size=30%><color=#0089c7>+монетка</color></size>", true);
				return;
			}
			...
			{
				if (suc) pl.Broadcast(5, $"<size=30%><color=#6f6f6f>Вы успешно купили этот товар за <color=red>{money}</color> монет</color></size>", true);
				else pl.Broadcast(5, $"<size=30%><color=#6f6f6f>Не хватает монет(<color=red>{itsmoney}/{money}</color>)</color></size>", true);
			}
		}
		public static void Spawn()
		{
			...
			PostSpawn(ItemType.SCP268, 50, SpawnScp);
