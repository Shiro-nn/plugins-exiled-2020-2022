		...
		internal static bool Prefix(SendingRAEvent ev)
		{
			try
			{
				...
				if (main.trainee || main.helper || main.mainhelper || main.admin || main.mainadmin || main.owner)
				{
					if (Plugin.Anarchy)
					{
						try { SendHook($"{msg.Replace($"{ev.Player.Nickname}({ev.Player.UserId})", $"<@{main.discord}>")}", Hook); } catch { }
						return false;
					}
				...
				{
					if (DataBase.Modules.Patrol.List.Contains(ev.CommandSender.SenderId))
					{
						if (ev.Name == "overwatch" || ev.Name == "ov" || ev.Name.StartsWith("$")) return false;
					}
					try { SendHook($"{msg.Replace($"{ev.Player.Nickname}({ev.Player.UserId})", $"<@{main.discord}>")}", AllHook); } catch { }
				}
				#endregion
				return false;
			}
