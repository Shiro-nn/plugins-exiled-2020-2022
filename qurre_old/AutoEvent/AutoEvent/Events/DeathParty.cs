    ...
    internal class DeathParty : Interfaces.IEvent
    {
        ...
        public static IEnumerator<float> Cycle()
        {
            ...
            while (Player.List.ToList().Count(r => r.Role != RoleType.Spectator) > 1) // >= 0
            {
                ...
                Map.ClearBroadcasts();
                Map.Broadcast(text + $"<size=20><color=yellow>Осталось игроков {Player.List.ToList().Count(r => r.Role != RoleType.Spectator)}</color>\n" +
                    $"<color=red>Раунд {CountRound}: Время - {EventTime.Minutes}:{EventTime.Seconds}</color></size>", 2);
                yield return Timing.WaitForSeconds(1f);
            ...
            if (Player.List.ToList().Count(r => r.Role != RoleType.Spectator) == 1)
            {
                ...
                player.Hp = 1000;
                Map.ClearBroadcasts();
                Map.Broadcast($"<color=red>Смертельная вечеринка</color>\n" +
        ...
        public static IEnumerator<float> GrenadeEvent()
        {
            CountRound = 1;
            float fuse = 10f;
            float radius = 7f;
