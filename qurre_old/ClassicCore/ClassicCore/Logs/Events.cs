...
namespace ClassicCore.Logs
{
	...
	internal static class RALogsPatch
	{
		...
		internal static bool Prefix(SendingRAEvent ev)
		{
			try
			{
				...
				{
					ev.Allowed = false;
					string message = $"{PlayerManager.players.Count}/{Init.MaxPlayers}\n";
					foreach (Player player in Player.List) message += $"{player.Nickname} - {player.UserId} ({player.Id}) [{player.Role}]\n";
				...
				else if (ev.Name.ToLower() == "stafflist")
				{
					...
					ev.CommandSender.RaReply($"{PlayerManager.players.Count}/{Init.MaxPlayers}\n{response}", true, true, string.Empty);
				}
				#endregion
				#region logs
	...
	{
		internal static bool _waiting = false;
		internal static bool _round = false;
		static internal void Waiting()
		{
			if (_round) Api.SendRa($"⌛ Ожидание игроков...", Api.Status.Donate);
			_waiting = false;
			_round = false;
		}
		static internal void RoundStart()
		{
			if (_waiting) Api.SendRa($"▶️ Раунд запущен: {Player.List.Count()} игроков на сервере.", Api.Status.Donate);
			_waiting = false;
			_round = false;
		}
		static internal void RoundEnd(RoundEndEvent _)
		{
			if (_round) Api.SendRa($"⏹️ Раунд закончен: {Player.List.Count()} игроков онлайн.", Api.Status.Donate);
		}
	}
}