...
namespace scp682343
{
	partial class EventHandlers
	{
		...
		{
			RaycastHit info;
			if (Physics.Linecast(ev.Shooter.GetComponent<Scp049_2PlayerScript>().plyCam.transform.position, ev.TargetPos, out info))
			{
			...
			else if (doorInt.Player.queryProcessor.PlayerId == scp343?.queryProcessor.PlayerId)
			{
				...
				else if (!RoundSummary.RoundInProgress() || (double)(float)RoundSummary.roundTime < (double)Configs.initialCooldown)
				{
					if (!doorInt.Allow)
					{
						...
						doorInt.Player.Broadcast(string.Format(Configs.dontaccess, (object)(float)(double)i), 1, false);
