...
namespace Loli.Modules
{
	public partial class EventHandlers
	{
		...
		public void Tesla(TeslaTriggerEvent ev)
		{
			if (Plugin.RolePlay)
			{
				if (ev.Player.Role is RoleType.Scientist || ev.Player.Team is Team.MTF) ev.Allowed = false;
			}
			else
			{
		...
		internal void CheckEscape(Player pl)
		{
			...
			if (pl is not null && pl.Escape is not null && Vector3.Distance(pl.Position, pl.Escape.worldPosition) < global::Escape.radius)
			{
				...
				if (!_cuffed && !Plugin.RolePlay && pl.GetCustomRole() is Module.RoleType.FacilityGuard)
				{
		...
		internal void SinkHole(SinkholeWalkingEvent ev)
		{
			...
			else if (ev.Event == HazardEventsType.Exit && ev.Player.Position.y.Difference(-2000) > 50)
			{
			...
			new Thread(() =>
			{
				...
				*/
				...
				bool LowHourse(out bool hardcheck)
				{
					...
					{
						if (ev.Player.UserId.Contains("@discord")) return false;
						var url = "https://" + "api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=C6080922CC0D5C8AE73B1C2499805ED5&steamid=" +
							ev.Player.UserId.Replace("@steam", "").Replace("@discord", "");
						var request = WebRequest.Create(url);
						request.Method = "GET";
						using var webResponse = request.GetResponse();
						...
						var data = reader.ReadToEnd();
						hardcheck = true;
						if (!data.Contains("games")) return Plugin.RolePlay;
