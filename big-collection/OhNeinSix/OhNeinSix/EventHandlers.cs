	...
	public class EventHandlers
	{
		...
		private IEnumerator<float> GetClosestPlayer(Scp096PlayerScript script, ReferenceHub player)
		{
			...
			while (script.Networkenraged == Scp096PlayerScript.RageState.Enraged)
			{
				...
				player.Broadcast(1, "<size=30><color=#c50000>До ближайшего таргета: </color><color=#10F110>" + bar + "</color></size> \n" + "<size=25>Таргетов: <color=#c50000>" + Plugin.Scp096Targets.Count + "</color></size>");
		...
		{
			if (ev.Player.GetRole() == RoleType.Tutorial)
			{
				ev.Allow = false;
				return;
			}
		...
		private void GetValidTargets(ReferenceHub scp)
		{
			foreach (ReferenceHub hub in Player.GetHubs())
			{
				...
				hub.Broadcast(5, "Вы-жертва SCP-096!");
			}
		}
