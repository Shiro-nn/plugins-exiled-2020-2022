...
namespace Loli.DataBase
{
	...
	static class Shop
	{
		...
		static void Pickup(PickupItemEvent ev)
		{
			...
			if (!Data.Users.TryGetValue(ev.Player.UserInfomation.UserId, out var _main)) return;
			if (ev.Pickup.Type == ItemType.Coin)
			{
				ev.Allowed = false;

                Stats.AddMoney(ev.Player, 1);
				ev.Pickup.Destroy();
			...
			void Postfix(Player pl, int money, ItemType item)
			{
				...
				{
                    Stats.AddMoney(ev.Player, 0 - money);
		...
		{
			Vector3 SpawnCard = Respawn.GetPosition(RoleTypeId.Scp049);
			Vector3 SpawnGun = Respawn.GetPosition(RoleTypeId.Scp939);
			Vector3 SpawnMed = Respawn.GetPosition(RoleTypeId.Scp173);
			Vector3 SpawnScp = Respawn.GetPosition(RoleTypeId.Scp096);
			Vector3 SpawnEtc = Respawn.GetPosition(RoleTypeId.FacilityGuard);
			if (UnityEngine.Random.Range(1, 100) < 20)
			{
				SpawnCard = Respawn.GetPosition(RoleTypeId.FacilityGuard);
				SpawnGun = Respawn.GetPosition(RoleTypeId.Scp049);
				SpawnMed = Respawn.GetPosition(RoleTypeId.Scp939);
				SpawnScp = Respawn.GetPosition(RoleTypeId.Scp173);
				SpawnEtc = Respawn.GetPosition(RoleTypeId.Scp096);
				Map.Broadcast($"<size=30%><color=#31d400>Магазин появился на <color=red>случайном</color> спавне</color></size>", 10);
			}
			else if (UnityEngine.Random.Range(1, 100) < 40)
			{
				SpawnCard = Respawn.GetPosition(RoleTypeId.Scp096);
				SpawnGun = Respawn.GetPosition(RoleTypeId.FacilityGuard);
				SpawnMed = Respawn.GetPosition(RoleTypeId.Scp049);
				SpawnScp = Respawn.GetPosition(RoleTypeId.Scp939);
				SpawnEtc = Respawn.GetPosition(RoleTypeId.Scp173);
				Map.Broadcast($"<size=30%><color=#ffb500>Магазин появился на <color=red>случайном</color> спавне</color></size>", 10);
			}
			else if (UnityEngine.Random.Range(1, 100) < 60)
			{
				SpawnCard = Respawn.GetPosition(RoleTypeId.Scp939);
				SpawnGun = Respawn.GetPosition(RoleTypeId.Scp096);
				SpawnMed = Respawn.GetPosition(RoleTypeId.FacilityGuard);
				SpawnScp = Respawn.GetPosition(RoleTypeId.Scp049);
				SpawnEtc = Respawn.GetPosition(RoleTypeId.Scp173);
				Map.Broadcast($"<size=30%><color=#fdffbb>Магазин появился на <color=red>случайном</color> спавне</color></size>", 10);
			}
			else if (UnityEngine.Random.Range(1, 100) < 80)
			{
				SpawnCard = Respawn.GetPosition(RoleTypeId.Scp939);
				SpawnGun = Respawn.GetPosition(RoleTypeId.Scp096);
				SpawnMed = Respawn.GetPosition(RoleTypeId.Scp173);
				SpawnScp = Respawn.GetPosition(RoleTypeId.FacilityGuard);
				SpawnEtc = Respawn.GetPosition(RoleTypeId.Scp049);
