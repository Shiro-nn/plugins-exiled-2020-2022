...
namespace Juggernaut
{
	partial class EventHandlers
	{
		...
		public void OnPlayerDie(ref PlayerDeathEvent ev)
		{
			...
			if (ev.Killer.queryProcessor.PlayerId == Juggernaut?.queryProcessor.PlayerId)
			{

				Timing.CallDelayed(0.5f, () => Juggernaut.SetRank("Джаггернаут", "magenta"));
			}
		}
		...
		public void OnCheckEscape(ref CheckEscapeEvent ev)
		{
			if (ev.Player.queryProcessor.PlayerId == Juggernaut?.queryProcessor.PlayerId)
			{
				...
				ev.Player.Broadcast($"<color=red>Вы не можете сбежать!</color>", 10);
			}
		}
		...
		internal void RemoteAdminCommand(ref RACommandEvent ev)
		{
			...
			if (command[0] == "JG")
			{
				if (player == null)
				{
					ev.Sender.RAMessage("хмм, игрок не найден, жаль(");
					return;
				}
				ev.Allow = false;
