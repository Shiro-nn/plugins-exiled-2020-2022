	...
	static class Loader
	{
		...
		static void InitLoader()
		{
			Core.Socket.On("database.get.data", obj =>
			{
				...
				UserData json = JsonConvert.DeserializeObject<UserData>(obj[1].ToString());
		...
		static internal void LoadData(Player pl)
		{
			...
			Core.Socket.Emit("database.get.data", new object[] { pl.UserInfomation.UserId, pl.UserInfomation.Ip });
		}
		static void LoadRoles(Player player, UserData data)
		{
			CustomDonates.CheckGetDonate(player, data.steam);
			Core.Socket.Emit("database.get.donate.ra", new object[] { player.UserInfomation.UserId });
			Core.Socket.Emit("database.get.donate.roles", new object[] { player.UserInfomation.UserId });
			Core.Socket.Emit("database.get.donate.customize", new object[] { player.UserInfomation.UserId });
			if (data.administration.moderator)
			{
				player.Administrative.ServerRoles.SetGroup(ServerStatic.GetPermissionsHandler().GetGroup("moderator"), false, true, false);
				...
				ServerStatic.GetPermissionsHandler()._members.Add(player.UserInfomation.UserId, "moderator");
				Levels.SetPrefix(player);
			}
			if (data.administration.admin)
			{
		...
		internal static void InitializeSocket()
		{
			Core.Socket.On("ChangeFreezeSCPServer", data =>
			{
				...
				foreach (var user in Data.Users.Where(x => x.Key is not null && x.Value is not null && x.Value.steam == doc.owner))
				{
					...
					if (Data.Roles.TryGetValue(user.Key, out var role))
					{
						if (doc.freezed)
						{
                            switch (doc.id)
                            {
								...
								case 2: role._primes--; break;
								case 3: role._mages--; break;
								case 4: role._sages--; break;
								case 5: role._stars--; break;
							}
						}
						else
						{
		...
		static internal void UpdateRole(Player pl)
		{
			...
			{
				if (!Data.Roles.TryGetValue(pl.UserInfomation.UserId, out var data)) return;
				if (pl.Administrative.RemoteAdmin && !(data.Mage || data.Sage || data.Star))
				{
					if (pl.UserInfomation.UserId.Contains("@northwood")) return;
					if (Data.Users.TryGetValue(pl.UserInfomation.UserId, out var data2) && (data2.donater || 
						data2.administration.owner || data2.administration.admin || data2.administration.moderator)) return;
			...
			Core.Socket.On("database.get.donate.roles", obj =>
			{
				...
				foreach (int role in roles)
				{
					switch (role)
					{
						...
						case 3: data._mages++; break;
						case 4: data._sages++; break;
						case 5: data._stars++; break;
					}
				}
				UpdateRole(pl);
				try { Levels.SetPrefix(pl); } catch { }
