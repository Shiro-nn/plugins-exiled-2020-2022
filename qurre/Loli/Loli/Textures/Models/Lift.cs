    ...
    internal class Lift
    {
        ...
        internal Lift(LiftCreator first, LiftCreator second, Color color, bool stopAlpha = false)
        {
            ...
            LocalCreator Create(LiftCreator cre, bool first)
            {
                ...
                {
                    Door door = new(Model.GameObject.transform.position + Vector3.down * 2.46f, DoorPrefabs.DoorHCZ, Model.GameObject.transform.rotation)
                    {
                        Name = LiftDoorName
                    };
        ...
        internal void Interact()
        {
            ...
            IEnumerator<float> ToFirst()
            {
                ...
                if (StopAlpha && Alpha.Detonated)
                {
                    foreach (var pl in Player.List.Where(x => InLift(x.MovementState.Position)))
                        try { pl.HealthInfomation.Kill(PlayerStatsSystem.DeathTranslations.Warhead); } catch { }
                    yield break;
                }
                foreach (var pl in Player.List.Where(x => InLift(x.MovementState.Position)))
                    try
                    {
                        pl.ReferenceHub.TryOverridePosition(pl.MovementState.Position + Objects.FirstPositions.Center - Objects.SecondPositions.Center,
            ...
            IEnumerator<float> ToSecond()
            {
                ...
                foreach (var pl in Player.List.Where(x => InLift(x.MovementState.Position)))
                    try
                    {
                        pl.ReferenceHub.TryOverridePosition(pl.MovementState.Position + Objects.SecondPositions.Center - Objects.FirstPositions.Center,
