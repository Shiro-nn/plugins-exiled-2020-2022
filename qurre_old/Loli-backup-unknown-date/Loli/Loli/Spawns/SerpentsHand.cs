...
namespace Loli.Spawns
{
	public class SerpentsHand
	{
		...
		public static List<Player> SerpentsHands => Player.List.Where(x => x.Tag.Contains(HandTag) && x.Role == RoleType.Tutorial).ToList();
		...
		public static bool ItsAliveHand(Player pl) => pl.Tag.Contains(HandTag) && pl.Role == RoleType.Tutorial && Modules.EventHandlers.Pos[pl.UserId].Alive;
		public void Spawn()
		{
			...
			already = true;
			if (Player.List.Where(x => x.Role == RoleType.Spectator && !x.Overwatch).ToList().Count == 0) return;
			...
			IEnumerator<float> SpawnPost()
			{
				while (!spawned)
				{
					if (0 >= toSpawn)
					{
						if (Player.List.Where(x => x.Role == RoleType.Spectator && !x.Overwatch).ToList().Count != 0)
						{
						...
						int cc = newsh.Count;
						if (newsh.Count > 10) newsh = newsh.Where(x => !Blocked(x)).ToList();
						...
						newsh.Shuffle();
						newsh = newsh.Take(10).ToList();
						try { SCPDiscordLogs.Api.SendMessage($"<:serpents_hand:840582086544064543> Приехал отряд длань змеи в кол-ве {newsh.Count()} человек."); } catch { }
						foreach (Player sh in newsh) SpawnOne(sh);
						spawned = true;
						yield break;
					}
					else if (Player.List.Where(x => x.Role == RoleType.Spectator && !x.Overwatch).ToList().Count != 0)
					{
		...
		internal void Damage(DamageProcessEvent ev)
		{
			...
			if ((ev.Target.Tag.Contains(HandTag) && (ev.Attacker.GetTeam() == Team.SCP || ev.DamageType == DamageTypes.Pocket)) ||
				(ev.Attacker.Tag.Contains(HandTag) && ev.Target.GetTeam() == Team.SCP) ||
			...
			else if (ev.Target.Tag.Contains(HandTag) && ev.Attacker.GetTeam() == Team.SCP)
			{
				ev.Allowed = false;
				ev.Amount = 0f;
			}
			else if (ev.Attacker.Tag.Contains(HandTag) && ev.Target.GetTeam() == Team.SCP)
			{
				ev.Allowed = false;
				ev.Amount = 0f;
			}
