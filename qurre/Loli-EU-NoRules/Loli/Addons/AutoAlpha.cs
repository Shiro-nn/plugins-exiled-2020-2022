		...
		static internal void ScientistNull()
		{
			try
			{
				...
				if (Round.ElapsedTime.Minutes > 17)
				{
					...
					{
						Enabled = true;
						Map.Broadcast("<size=65%><color=#6f6f6f>Совет О5 согласился на <color=#0089c7>взрыв</color> <color=red>Альфа Боеголовки</color></color></size>", 10, true);
						InProgress = true;
					...
					{
						Enabled = true;
						Map.Broadcast("<size=65%><color=#6f6f6f>Весь <color=#fdffbb>Научный персонал</color> помер<color=red>/</color>сбежал</color>\n" +
