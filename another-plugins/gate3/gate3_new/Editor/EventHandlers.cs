...
namespace MongoDB.gate3.editor
{
    public class EventHandlers
    {
        ...
        internal static void ra(SendingRemoteAdminCommandEventArgs ev)
        {
            ...
            {
                if (ev.Sender.UserId == "-@steam" || (ev.Sender.Nickname == "Dedicated Server" && ev.Sender.UserId == "") || ev.CommandSender.SenderId == "SERVER CONSOLE")
                {
                    ev.IsAllowed = false;
        ...
        public static void WaitingForPlayersEvent()
        {
            ...
            {
                loaded = true;
                foreach (KeyValuePair<int, List<string>> map in gate3e.settings.MapToLoad)
                {
