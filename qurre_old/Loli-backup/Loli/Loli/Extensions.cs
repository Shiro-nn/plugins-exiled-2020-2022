...
namespace Loli
{
	public static class Extensions
	{
		...
		public static float GetMaxHp(this Player pl)
		{
			...
			else if (Plugin.RolePlay && pl.Team is not Team.SCP) maxhp = UnityEngine.Random.Range(100, 135);
			else switch (pl.Role)
				{
					...
					case RoleType.Scp106:
						if (Plugin.RolePlay) maxhp = 2500;
						else maxhp = pl.ClassManager.Classes.SafeGet(RoleType.Scp106).maxHP;
						break;
		...
		public static void GetAmmo(this Player pl)
		{
			if (Plugin.RolePlay)
			{
				if (pl.Team is Team.CHI or Team.MTF or Team.TUT)
				{
				...
				else if (pl.Team is Team.RSC) pl.AddItem(ItemType.Ammo9x19);
			}
			else
			{
				pl.Ammo12Gauge = 999;
