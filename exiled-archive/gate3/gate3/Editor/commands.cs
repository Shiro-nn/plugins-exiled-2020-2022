...
namespace MapEditor
{
	...
	public class CommandEditor
	{
		...
		public static void PrepareMap()
		{
			...
			{
				string path = Path.Combine(MainClass.pluginDir, "maps", text + ".yml");
				bool flag = !File.Exists(path);
				if (!flag)
				{
					...
					foreach (Editor.MapObject mapObject in objects)
					{
						...
						mapObjectLoaded.workStation = MainClass.GetWorkStationObject();
						...
						mapObjectLoaded.workStation.AddComponent<WorkStationUpgrader>();
				...
				if (!flag)
				{
					...
					if (!flag2)
					{
						...
						if (isWorkstation)
						{
							...
							mapObjectLoaded2.workStation = MainClass.GetWorkStationObject();
							...
							mapObjectLoaded2.workStation.AddComponent<WorkStationUpgrader>();
		...
		public static void SaveMap(CommandSender sender)
		{
			...
			else
			{
				...
				string path = Path.Combine(MainClass.pluginDir, "maps", mapName + ".yml");
				File.WriteAllText(path, serializer.Serialize(new Editor.YML
				{
					objects = list
				}));
