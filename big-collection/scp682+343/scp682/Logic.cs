	...
	partial class EventHandlers
	{
		...
		private static void Killscp343(bool setRank = true)//kill scp 343
		{
			if (setRank)
			{
				...
				GameCore.Console.singleton.TypeCommand($"/god {scp343.GetPlayerId()}. 0");
				scp343 = null;
			}
		}
		...
		public static void SpawnJG(ReferenceHub JG)//2 events for correct spawn
		{
			    ...
			    maxHP = JG.playerStats.maxHP;
			    Cassie.CassieMessage(Configs.scpspawnripcassie, false, false);
				Timing.CallDelayed(0.5f, () => JG.playerStats.maxHP = Configs.health);
				Timing.CallDelayed(0.5f, () => JG.playerStats.health = Configs.health);
				JG.ClearBroadcasts();
		...
		public static void Spawn343(ReferenceHub sss)//spawn scp 343
		{
				...
				Timing.CallDelayed(0.5f, () => sss.ammoBox.Networkamount = "0:0:0");
				sss.effectsController.EnableEffect("SCP-207");
			    scp343 = sss;
		}
		...
		public static void selectspawnJG()//2 events for correct spawn
		{
			...
			if (true)
			{
				...
				if (pList.Count > Configs.count && scp682 == null)
				{
