...
namespace Loli.Addons.RolePlay.CutScenes
{
    ...
    internal class Scp106Scene
    {
        ...
        {
            if (Plugin.RolePlay && ev.Player.Tag.Contains("Custom106Pos") && !Plugin.HardRP) ev.Position = Map.GetRandomSpawnPoint(RoleType.Scp106);
        }
        internal void FixTP(DroppingItemEvent ev)
        {
            if (Plugin.RolePlay && ev.Player.Tag.Contains("Custom106Pos") && !Plugin.HardRP) ev.Allowed = false;
        }
        ...
        {
            if (Plugin.RolePlay && ev.Target.Tag.Contains("Custom106Pos") && !Plugin.HardRP) ev.Allowed = false;
        }
        ...
        {
            if (!Plugin.RolePlay || Plugin.HardRP) return;
            ...
            if (pl106 != null)
            {
                ...
                pl106.ChangeModel(RoleType.Scp106);
                pl106.Position = Map.GetRandomSpawnPoint(RoleType.Scp106);
            ...
            void Teleport()
            {
                ...
                for (int i = 0; i < list.Count(); i++)
                {
                    ...
                    else if (i == 1 && Random.Range(0, 100) < 40) SpawnManager(pl);
                    else BackSpawn(pl);
                }
                void SpawnMajor(Player pl)
                {
                    See(pl, "<color=#fdffbb>Координатор исследований</color>", "лайт-зоне");
                    Timing.CallDelayed(28f, () =>
                    {
                        pl.Invisible = false;
                        pl.Tag = pl.Tag.Replace("Custom106Pos", "");
                        Scientists.SpawnMajor(pl);
                    });
                }
                void SpawnManager(Player pl)
                {
                    See(pl, "<color=#ff0000>Менеджер комплекса</color>", "офисной зоне");
                    Timing.CallDelayed(28f, () =>
                    {
                        pl.Invisible = false;
                        pl.Tag = pl.Tag.Replace("Custom106Pos", "");
                        FacilityManager.Spawn(pl);
                    });
                }
                void BackSpawn(Player pl)
                {
                    See(pl, "<color=#e2e26d>Научный сотрудник</color>", "лайт-зоне");
                    Timing.CallDelayed(28f, () =>
                    {
                        if (round != Round.CurrentRound) return;
                        pl.Invisible = false;
                        pl.Tag = pl.Tag.Replace("Custom106Pos", "");
                        Scientists.SpawnRandomSafe(pl);
                    });
                }
                void See(Player pl, string role, string loc)
                {
                    pl.Tag += "Custom106Pos";
                    pl.Invisible = true;
                    pl.Role = RoleType.Tutorial;
                    pl.ChangeModel(RoleType.Scientist);
                    pl.Position = Map.GetRandomSpawnPoint(RoleType.Scp106);
                    pl.Broadcast($"<size=30%><color=#6f6f6f>Вы - {role}\n" +
                        $"После катсцены, вы появитесь в {loc}</color></size>", 10, true);
            ...
            void DoSpawn(Vector3 sciPos, Vector3 sciRot, Vector3 scpPos, Vector3 portalPos)
            {
                ...
                Timing.CallDelayed(9f, () =>
                {
                    ...
                    IEnumerator<float> DoRun106()
                    {
                        ...
                        NetworkServer.Spawn(gameObject);
                        yield return Timing.WaitForSeconds(0.5f);
                        var chps = Qurre.API.Extensions.GetRoom(RoomType.HczEzCheckpoint).Position;
                        bot106.Player.Scp106Controller.PortalPosition = chps;
                        new Ragdoll(RoleType.Scp106, chps + Vector3.up * 2, Quaternion.identity, new CustomReasonDamageHandler("Нарушил ВОУС?"), "SCP-106", 0)
                        {
                            Scale = new(3, 0.1f, 3)
                        };
                        new Ragdoll(RoleType.Scp106, scpPos + Vector3.up * 2, Quaternion.identity, new CustomReasonDamageHandler("Нарушил ВОУС?"), "SCP-106", 0)
                        {
                            Scale = new(10, 0.1f, 10)
                        };
                        yield return Timing.WaitForSeconds(0.5f);
                        bot106.Player.Scp106Controller.UsePortal();
                        if (pl106 == null)
                ...
                Timing.CallDelayed(14f, () =>
                {
                    ...
                    IEnumerator<float> CheckSinkHole()
                    {
                        for (; ;)
                        {
                            if (Vector3.Distance(botSci.Position, portalPos) < 2f || Round.ElapsedTime.TotalSeconds > 22)
                            {
                                ...
                                Ragdoll.Create(RoleType.Scientist, botSci.Position, Quaternion.Euler(botSci.Rotation),
