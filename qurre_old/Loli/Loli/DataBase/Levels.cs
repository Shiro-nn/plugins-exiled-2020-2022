...
namespace Loli.DataBase
{
	internal class Levels
	{
		...
		private DateTime LastPay = DateTime.Now;
		private void ConsolePay(SendingConsoleEvent ev)
		{
			ev.Allowed = false;
			if (ev.Args.Count() < 2)
			{
				ev.ReturnMessage = $"Команда введена неверно\nПример: {ev.Name} 10 hmm";
				ev.Color = "red";
				return;
			}
			...
			Player pl = Player.Get(search);
			if (pl == null)
			{
				ev.ReturnMessage = $"Игрок \"{search}\" не найден";
				ev.Color = "red";
				return;
			}
			if (pl.UserId == ev.Player.UserId)
			{
				ev.ReturnMessage = "Нельзя передать монетки самому себе";
				ev.Color = "red";
				return;
			}
			if ((DateTime.Now - LastPay).TotalSeconds < 0.5)
			{
				ev.ReturnMessage = "Рейт-лимит, попробуйте снова";
				ev.Color = "red";
				return;
			}
			LastPay = DateTime.Now;
			{
				Plugin.Socket.Emit("database.get.stats", new object[] { ev.Player.UserId.Replace("@steam", "").Replace("@discord", ""),
							ev.Player.UserId.Contains("discord"), ev.Player.UserId+"+updating"});
				string uid = "";
				uid = Plugin.Socket.On("database.get.stats", obj => Timing.CallDelayed(0.5f, () => DoUpdate(obj)));
				void DoUpdate(object[] obj)
				{
					string userid = obj[1].ToString();
					if (userid != ev.Player.UserId + "+updating") return;
					var sender = Player.Get(userid.Replace("+updating", ""));
					if (sender is null) return;
					Plugin.Socket.Off(uid);
					...
					if (json.money >= result)
					{
						...
						ev.Player.SendConsoleMessage($"Вы успешно передали {result} монет игроку {pl.Nickname}", "white");
						pl.SendConsoleMessage($"{ev.Player.Nickname} передал вам {result} монет", "white");
						return;
					}
					ev.Player.SendConsoleMessage($"Не хватает монет({json.money}/{result})", "red");
				}
			}
			ev.ReturnMessage = "Запрос на перевод создан";
		}
		...
		public void XpDeath(DeadEvent ev)
		{
			try
			{
				...
				try { if (EventHandlers.Upgrade914.ContainsKey(ev.Target.UserId) && EventHandlers.Upgrade914[ev.Target.UserId] && ev.Target.Role == RoleType.ClassD) return; } catch { }
		...
		private void GetKillXp(Player target, Player killer)
		{
			...
			killer.SendConsoleMessage($"Вы получили {LvlUp}xp & {moneyUp} монет за убийство {targetname}, который был {targetTeam}", "white");
		...
		private void GetHelpXp(Player target, Player killer)
		{
			...
			GetTotalXp(killer, ref LvlUp);
			killer.Hint(new(16, 6, $"<align=right><color=#fdffbb>+{LvlUp}xp & {moneyUp} 💰", 5, false));
			killer.SendConsoleMessage($"Вы получили {LvlUp}xp & {moneyUp} монет за помощь в убийстве {targetname}, который был {targetTeam}", "white");
			Manager.Static.Stats.Add(killer, (int)Math.Round(LvlUp), (int)Math.Round(moneyUp));
		}
		internal static void SetPrefix(Player player)
		{
			...
			try
			{
				...
				if (!imain.anonym)
				{
					...
					if (Modules.CustomDonates.ThisYt(imain) || imain.id == 1385 || imain.id == 3947)
					{
						color = "red";
						role += $" | YouTube";
					}
