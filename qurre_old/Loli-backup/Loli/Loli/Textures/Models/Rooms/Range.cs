...
namespace Loli.Textures.Models.Rooms
{
    internal class Range
    {
        ...
        internal static void GetCheaters()
        {
            ...
            {
                if (Cheaters.Contains(pl.UserId)) continue;
                try
                {
                    if (DataBase.Manager.Static.Data.Users.TryGetValue(pl.UserId, out var _data) &&
            ...
            void SendHook(Player pl)
            {
                new System.Threading.Thread(() =>
                {
                    ...
                    Embed embed = new()
                    {
                        ...
                        Color = 1,
                        Description = $"{pl.Nickname} - {pl.UserId} ({pl.Ip})",
        ...
        internal static void ClearWaitInv()
        {
            ...
            foreach (var pl in Player.List)
            {
                ...
                {
                    pl.ClearInventory();
                    pl.Inventory.UserInventory.ReserveAmmo.Clear();
                    pl.Inventory.SendAmmoNextFrame = true;
                }
                catch { }
            }
