...
namespace Loli.DataBase.Modules.Controllers
{
    internal class Nimb
    {
        ...
        internal Nimb(Player pl)
        {
            ...
            Color col = new(4, 4, 0);
            Model = new("Nimb", _pos);
            Model.GameObject.transform.parent = pl.MovementState.Transform;
        ...
        internal static void Update(EffectDisabledEvent ev)
        {
            ...
            try { obj.Model.GameObject.transform.localPosition = new Vector3(0, 1.28f * ev.Player.MovementState.Scale.y, 0); } catch { }
        }
        ...
        static void RoleChange(SpawnEvent ev)
        {
            ...
            if (!TryGet(ev.Player, out var obj)) return;
            try { obj.Model.GameObject.transform.parent = ev.Player.MovementState.Transform; } catch { }
            try { obj.Model.GameObject.transform.localPosition = new Vector3(0, 1.28f * ev.Player.MovementState.Scale.y, 0); } catch { }
        }
