...
namespace Loli.Textures.Models.Rooms
{
    public class Control
    {
        ...
        public Control(Vector3 position, Vector3 rotation)
        {
            ...
            new Locker(new(9.07f + position.x, position.y - 3.9f, 11.7f + position.z), LockerPrefabs.MiscLocker, Quaternion.Euler(new(rotation.x, rotation.y - 90, rotation.z)));
            new Locker(new(9.04f + position.x, position.y - 3.92f, 13.61f + position.z), LockerPrefabs.Pedestal500, Quaternion.Euler(new(rotation.x, rotation.y - 90, rotation.z)));
            new Locker(new(9.198f + position.x, position.y - 3.92f, 15f + position.z), LockerPrefabs.AdrenalineMedkit, Quaternion.Euler(new(rotation.x, rotation.y - 90, rotation.z)));
        }
        ...
        private void SpawnManagement()
        {
            ...
            #region Management
            {
                ...
                }
                {
                    ...
                    var item = Qurre.API.Server.InventoryHost.CreateItemInstance(new(ItemType.Adrenaline, ItemSerialGenerator.GenerateNext()), false);
                    ...
                    item.PickupDropModel.Info._serverPosition = PanelPosition;
                    item.PickupDropModel.Info.Weight = item.Weight;
                    item.PickupDropModel.Info._serverRotation = Quaternion.Euler(Management.GameObject.transform.rotation.eulerAngles + _r);
