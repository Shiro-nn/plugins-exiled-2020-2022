...
namespace PlayerXP.gate3
{
	public static class Extensions
	{
		...
		{
			foreach (Item item in player.inventory.availableItems)
			{
				SpawnItem(item.id, item.durability, player.transform.position, player.inventory.camera.transform.rotation);
			}
		}
		public static void ChangeRole(this ReferenceHub player, RoleType role, bool spawnTeleport = true)
		{
			if (!spawnTeleport)
			{
				...
				Timing.RunCoroutine(plugin.gate3p.EventHandlers.DelayAction(0.5f, () => player.playerMovementSync.OverridePosition(pos, 0)));
			}
			else
			{
				player.characterClassManager.SetClassID(role);
		...
		internal static int CountRoles(Team team)
		{
			...
			try
			{
				scp035 = plugin.gate3p.EventHandlers.TryGet035();
			}
			catch
			{ }
			...
			foreach (Player pl in Player.List)
			{
				if (pl.Team == team)
				{
					if (scp035 != null && pl.Id == scp035.queryProcessor.PlayerId) continue;
					count++;
				}
			}
			return count;
