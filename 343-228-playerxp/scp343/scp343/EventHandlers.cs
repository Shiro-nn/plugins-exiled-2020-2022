...
namespace scp343
{
	partial class EventHandlers
	{
		...
		public void OnShoot(ref ShootEvent ev)
		{
			...
			{
				ev.Shooter.weaponManager.GetShootPermission(Team.CDP, true);
				ev.Shooter.weaponManager.GetShootPermission(Team.CHI, true);
		...
		public void RunOnRACommandSent(ref RACommandEvent RACom)
		{
			try
			{
				...
				if (RACom.Command.ToLower().StartsWith("scp343"))
				{
					RACom.Allow = false;
			...
			catch
			{
				...
				{
					RACom.Allow = false;
					RACom.Sender.RAMessage("Произошла ошибка");
					return;
				}
			}
		}
