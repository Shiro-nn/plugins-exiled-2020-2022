...
namespace Loli.Spawns.Roles
{
	internal static class Hacker
	{
		...
		internal static void SpawnHacker(Player pl, Player guard = null)
		{
			...
			if (guard is null)
			{
				var list = Player.List.Where(x => x.Role is RoleType.Spectator && !x.Overwatch).ToArray();
			...
			Timing.CallDelayed(0.3f, () =>
			{
				...
				if (Plugin.RolePlay)
				{
					...
					pl.Ahp = Round.ElapsedTime.TotalMinutes switch
					{
						< 5 => 50,
						< 10 => 100,
						< 15 => 150,
		...
		{
			if (ev.RoleType.GetTeam() is not Team.CHI) return;
		...
		{
			if (ev.NewRole is RoleType.ChaosRepressor) return;
		...
		{
			if (ev.RoleType is RoleType.ChaosRepressor) return;
			if (ev.Player.Tag.Contains(HackerTag))
			{
				ev.Player.Tag = ev.Player.Tag.Replace(HackerTag, "");
				ev.Player.AhpActiveProcesses.ForEach(x => x.DecayRate = 1);
