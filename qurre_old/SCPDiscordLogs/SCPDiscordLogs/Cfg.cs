...
namespace SCPDiscordLogs
{
	static public class Cfg
	{
		...
		static internal void LoadReloadCfg()
		{
			...
			BlockRa = Config.SafeGetValue("InvisibleRa", new string[] { "-@steam", "444@discord", "-@steam" },
				"Those who will not be logged in ra logs").Select(x => x.Trim()).ToList();
			GlobalInvisible = Config.SafeGetValue("GlobalInvisible", new string[] { "-@steam", "444@discord", "-@steam" },
