...
namespace Loli.DataBase
{
	...
	internal class Customize
	{
		...
		public Customize(Plugin plugin)
        {
			this.plugin = plugin;
			Static = this;
		}
		internal static readonly Dictionary<string, BdData> Customizes = new();
		internal void Get(Player pl, string web_name)
		{
			new Thread(() =>
			{
				try
				{
					var collection = Manager.Static.DataBase.MongoDatabase.GetCollection<BdData>("customizes");
					var list = collection.Find(x => x.owner == web_name && x.active).ToList();
					if (list.Count == 0) return;
					var data = list.First();
					if (Customizes.ContainsKey(pl.UserId)) Customizes.Remove(pl.UserId);
					Customizes.Add(pl.UserId, data);
				}
				catch (System.Exception e)
				{
					Qurre.Log.Error($"umm, error:\n{e}\n{e.StackTrace}");
				}
			}).Start();
		}
