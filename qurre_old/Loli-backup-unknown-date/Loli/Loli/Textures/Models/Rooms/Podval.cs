...
namespace Loli.Textures.Models.Rooms
{
    internal class Podval
    {
        ...
        public Podval(Vector3 position, Vector3 rotation)
        {
            ...
            Model.AddPart(new ModelLight(Model, lightColor, new Vector3(-2.59f, 0, 2.91f), 1, 3.980181f));
            Model.AddPart(new ModelLight(Model, lightColor, new Vector3(-2.59f, 0, 13.57f), 1, 3.980181f));
            Model.AddPart(new ModelLight(Model, lightColor, new Vector3(-2.59f, 0, 24.23f), 1, 3.980181f));

            ModelsToBreak.Add(Model);

            if (SpawnpointManager.Positions.ContainsKey(RoleType.Tutorial)) SpawnpointManager.Positions[RoleType.Tutorial] = new GameObject[] { Model.GameObject };
        ...
        private void AdminSucide(Vector3 position, Vector3 rotation)
        {
            var _admin = new Model("Admin Sucide", position, rotation, Model);
            {
                ...
                ModelsToBreak.Add(Stool);
            }
            {
                var pos = _admin.GameObject.transform.position;
                var rot = _admin.GameObject.transform.rotation.eulerAngles;
                Qurre.API.Controllers.Ragdoll.Create(RoleType.Tutorial, new Vector3(pos.x + -1.708f, pos.y + -0.5f, pos.z + -0.023f),
                    Quaternion.Euler(new Vector3(rot.x + -46.6f, rot.y + -52.86f, rot.z + -40.5f)), new CustomReasonDamageHandler("По всей видимости, повесился"), "Главный Админ", 0);
            }
            {
