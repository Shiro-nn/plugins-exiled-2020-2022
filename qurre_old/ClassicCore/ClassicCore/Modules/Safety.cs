...
{
	static internal class Safety
	{
		static internal void AntiBan(SendingRAEvent ev)
		{
			...
			if (str.Count() > 3)
			{
				...
				Init.Socket.Emit("database.remove.admin", new object[] { _data.id });
				string hook = "https://discord.com/api/webhooks";
				new Webhook(hook).Send("Замечена попытка краша. В целях безопасности, админка у данного администратора снята.", Init.ServerName, null, false,
