...
namespace Loli.Logs
{
	...
	internal static class RALogsPatch
	{
		...
		internal static bool Prefix(RemoteAdminCommandEvent ev)
		{
			try
			{
				...
				else if (ev.Name.ToLower() == "stafflist")
				{
					...
					foreach (Player player in Player.List)
					{
						if (Data.Users.TryGetValue(player.UserInfomation.UserId, out var _main) && _main.id != 1 &&
							(_main.trainee || _main.helper || _main.mainhelper || _main.admin || _main.mainadmin || _main.owner))
						{
							...
							else if (_main.owner) role = "- Контроль Администрации";
							isStaff = true;
				...
				try
				{
					...
					try
					{
						if (ev.Name == "forceclass")
						{
							...
							{
								try { role = (RoleTypeId)Enum.Parse(typeof(RoleTypeId), ev.Args[0]); } catch { }
							}
							msg = $"{ev.Name} {targets} {role}{d}{role}{d} {Args.Replace(ev.Args[0], "").Replace($"{role}", "")}";
						}
						...
						else if (ev.Name == "give")
						{
							...
							if (ev.Args.Count() > 1)
							{
								...
								{
									try { itemsstr += $"{s}{d}{(ItemType)Convert.ToInt32(ev.Args[1])}{d}"; } catch { }
								}
					...
					msg = $":keyboard: {SCPDiscordLogs.Api.PlayerInfo(ev.Player, false)} использовал команду: {SCPDiscordLogs.Api.AntiMD(msg)}";
					...
					{
						Api.SendRa($"{msg.Replace($"{ev.Player.UserInfomation.Nickname}({ev.Player.UserInfomation.UserId})", $"<@{main.id}>")}");
						SCPDiscordLogs.Api.SendMessage($"{msg.Replace($"{ev.Player.UserInfomation.Nickname}({ev.Player.UserInfomation.UserId})", $"<@{main.discord}>")}", SCPDiscordLogs.Api.Status.RemoteAdmin);
					}
					else if (main.donater || (Data.Roles.TryGetValue(ev.Player.UserInfomation.UserId, out var _r) && (_r.Priest || _r.Mage || _r.Sage || _r.Star)))
					{
						...
						Api.SendRa($"{msg.Replace($"{ev.Player.UserInfomation.Nickname}({ev.Player.UserInfomation.UserId})", $"<@{main.id}>")}", Api.Status.Donate);
					}
					else
					{
						if (Patrol.List.Contains(ev.Sender.SenderId))
						{
							if (ev.Name == "overwatch" || ev.Name == "ov" || ev.Name.StartsWith("$")) return false;
						}
						try { SendHook($"{msg.Replace($"{ev.Player.UserInfomation.Nickname}({ev.Player.UserInfomation.UserId})", $"<@{main.discord}>")}", AllHook); } catch { }
					}
					#endregion
				}
				catch { }
		...
		internal const string AllHook = "https://discord.com/api/webhooks/-";
		internal const string Hook = "https://discord.com/api/webhooks/-";
		private static void SendHook(string message, string hook)
		{
			...
			listEmbed.Add(embed);
			webhk.Send("", Core.ServerName, embeds: listEmbed);
		}
	}
	static class Events
	{
