...
namespace Loli.Scps
{
    public class Scp079Better
    {
        ...
        {
            if (ev.Player.Role == RoleType.Scp079) ev.Player.Broadcast("<size=25%><color=#6f6f6f>Вы можете посмотреть способности <color=red>SCP 079</color>, написав <color=#ffa300>.079</color> в консоли на <color=#0089c7>[<color=#00ff00>ё</color>]</color>.</color></size>", 10, true);
        }
        internal void Console(SendingConsoleEvent ev)
        {
            ...
            {
                ev.ReturnMessage = "Увы, но вы не являетесь SCP 079.";
                ev.Color = "red";
                return;
            }
            string type = ev.Args[0];
            if (type == "1")
            {
                float mana = 15;
                ...
                ev.Player.Scp079Controller.Camera = scps.First().Room.Cameras.First();
            }
            else if (type == "2")
            {
                float mana = 30;
            ...
            else if (type == "4")
            {
                ...
                room.LightColor = new Color(Random.Range(0, 255) / 255f, Random.Range(0, 255) / 255f, Random.Range(0, 255) / 255f);
                Timing.CallDelayed(40f, () => room.LightOverride = false);
                Timing.CallDelayed(40f, () => room.LightColor = FlickerableLightController.DefaultWarheadColor);
            }
            else if (type == "5")
            {
                float mana = 20;
                ...
                ev.Player.Scp079Controller.Energy -= mana;
                var room = ev.Player.Scp079Controller.Camera.Room;
                var to = Random.Range(0, 255) / 255f;
                room.LightColor = new Color(to, to, to);
                Timing.CallDelayed(40f, () => room.LightOverride = false);
                Timing.CallDelayed(40f, () => room.LightColor = FlickerableLightController.DefaultWarheadColor);
            }
            else if (type == "6")
            {
                ...
                var room = ev.Player.Scp079Controller.Camera.Room;
                if (room == null || room.Type == RoomType.Pocket || room.Type == RoomType.Surface)
                {
        ...
        private IEnumerator<float> GasRoom(Room room, Player scp)
        {
            ...
            {
                if ((door.Type != DoorType.HCZ_079_First && door.Type != DoorType.HCZ_079_Second) || Round.ActiveGenerators > 2)
                {
                    door.Open = true;
                    door.Locked = true;
                }
            }
            room.LightColor = new Color32(255, 163, 0, 255);
            ...
            foreach (var door in doors)
            {
                if ((door.Type != DoorType.HCZ_079_First && door.Type != DoorType.HCZ_079_Second) || Round.ActiveGenerators > 2) door.Locked = false;
            }
            room.LightColor = Color.white;
