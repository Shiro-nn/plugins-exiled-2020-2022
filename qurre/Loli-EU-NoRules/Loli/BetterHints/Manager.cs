    ...
    internal static class Manager
    {
        ...
        internal static void Cycle()
        {
            ...
            string HackerMenu()
            {
                ...
                string stances = "";
                var _ps = Builds.Models.Panel.Panels.Select(x => x.Value);
                ...
                if (Builds.Models.Panel.AllHacked)
                {
                    var _stts = Builds.Models.Rooms.Control.Status;
                    var _prcs = Builds.Models.Rooms.Control.Proccess;
                    string color = "#00ff19";
                    string cc = "#009c0f";
                ...
                if (Builds.Models.Rooms.Control.Proccess >= 100)
                {
                    var _stts = Builds.Models.Rooms.ServersManager.Status;
                    var _prcs = Builds.Models.Rooms.ServersManager.Proccess;
                    string color = "#00ff19";
                    string cc = "#009c0f";
                    if (_stts == HackMode.Hacking)
                    {
            ...
            foreach (Player pl in Player.List)
            {
                try
                {
                    ...
                    if (pl.RoleInfomation.Team == Team.FoundationForces)
                        str += "\n" + CustomUnits.GetAll(pl);
                    if (pl.ItsHacker()) str += HackerMenu();
                    //if (false) str += ClosingSoon(leng);
                    try
                    {
                        if (Scps.Scp294.API.DrinksManager.Drinks.TryGetValue(pl.Inventory.Base.CurItem.SerialNumber, out var drink))
                        {
            ...
            static List<string> ScpText()
            {
                ...
                foreach (Player _ in scps) text.Add($"<align=right><size=70%><alpha=#FF><b><color=#ff0000>" +
                    $"{_.UserInfomation.Nickname} - {_.RoleInfomation.Role} ({Math.Round(_.HealthInfomation.Hp)}/{Math.Round((float)_.HealthInfomation.MaxHp)}HP)" +
                    $"</color></b></size></align>");
                return text;
            }
        }
