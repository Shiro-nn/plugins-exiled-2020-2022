...
namespace AutoEvent
{
    internal class JailEvent : Interfaces.IEvent
    {
        ...
        public static List<Player> StartedMtf { get; set; } = new List<Player>(); // Лист только для начала ивента
        ...
        public void OnStart()
        {
            ...
            OnEventStarted();
            LogInvList(Qurre.Events.Player.RoleChange);
        }
        void LogInvList<T>(Qurre.Events.Modules.Main.AllEvents<RoleChangeEvent> ev)
        {
            if (ev is null)
            {
                Qurre.Log.Info("Event is null");
                return;
            }
            foreach (Qurre.Events.Modules.Main.AllEvents<RoleChangeEvent> handler in ev.GetInvocationList())
            {
                Qurre.Log.Info(handler.Method);
            }
        }
        ...
        public static void WaitingEvent()
        {
            // День недели новый
            DayWeek = RandomMessage.RandomItem();
            // Если отрыты двери, то возвращаем обратно
        ...
        public static IEnumerator<float> Cycle()
        {
            ...
            {
                Map.ClearBroadcasts();
                Map.Broadcast($"<color=yellow>Ивент <color=red><b><i>Тюрьма Саймона</i></b></color>\n" +
            ...
            while (Player.List.Count(r => r.Role == RoleType.ClassD) > 0 && Player.List.Count(r => r.Team == Team.MTF) > 0)
            {
                ...
                Map.ClearBroadcasts();
                Map.Broadcast($"<size=20><color=red>Тюрьма Саймона</color>\n" +
                    ...
                    $"<color=red>{EventTime.Minutes}:{EventTime.Seconds}</color></size>", 2);
                yield return Timing.WaitForSeconds(1f);
            }
            if (Player.List.Count(r => r.Team == Team.MTF) == 0)
            {
                Map.ClearBroadcasts();
                Map.Broadcast($"<color=red><b><i>Победа Заключенных</i></b></color>\n" +
                    $"<color=red>{EventTime.Minutes}:{EventTime.Seconds}</color>", 10);
            }
            if (Player.List.Count(r => r.Role == RoleType.ClassD) == 0)
            {
                Map.ClearBroadcasts();
                Map.Broadcast($"<color=blue><b><i>Победа Охранников</i></b></color>\n" +
                    $"<color=red>{EventTime.Minutes}:{EventTime.Seconds}</color>", 10);
            }
        ...
        public static Vector3 RandomPosition(bool isJailer)
        {
            Vector3 position = new Vector3(0, 0, 0);
            if (isJailer) // Если это Охранник
            {
        ...
        public static List<Vector3> DoorsPosition = new List<Vector3>()
        {
            // 1 этаж
        ...
        public static void CreatePrisonerDoors()
        {
        ...
        public static void CreateJailerDoors()
        {
        ...
        public static void OnShootEvent(ShootingEvent ev)
        {
        ...
        public static void OnInteractLocker(InteractLockerEvent ev)
        {
            /*
