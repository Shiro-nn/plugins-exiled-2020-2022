...
namespace Loli.DataBase.Modules
{
	internal class Donate
	{
		...
		internal Donate(Manager manager) => Manager = manager;
		...
		internal void Ra(SendingRAEvent ev)
		{
			...
			if (ev.Name == "forceclass")
			{
				...
				if (roles.Priest)
				{
					if (role == RoleType.Tutorial || role.GetTeam() == Team.SCP)
					{
						ev.Success = false;
				...
				if (!Plugin.Anarchy && role.GetTeam() == Team.SCP && scps > 2 && Player.List.Count() / scps < 6)
				{
					ev.Success = false;
				...
				int Scp173 = Player.List.Where(x => x.Role == RoleType.Scp173).Count();
				int Scp106 = Player.List.Where(x => x.Role == RoleType.Scp106).Count();
				int Scp049 = Player.List.Where(x => x.Role == RoleType.Scp049).Count();
				int Scp079 = Player.List.Where(x => x.Role == RoleType.Scp079).Count();
				int Scp096 = Player.List.Where(x => x.Role == RoleType.Scp096).Count();
				int Scp93989 = Player.List.Where(x => x.Role == RoleType.Scp93989).Count();
				int Scp93953 = Player.List.Where(x => x.Role == RoleType.Scp93953).Count();
			...
			if (ev.Name == "server_event")
			{
				...
				ev.Allowed = false;
				if ((DateTime.Now - LastCall).TotalSeconds < 60)
				{
					ev.Success = false;
					var w = Math.Round(60 - (DateTime.Now - LastCall).TotalSeconds);
					ev.ReplyMessage = $"Последний отряд был вызван менее минуты назад\nПодождите {w} сек";
					return;
				}
				if (ev.Args[0].ToLower() == "force_mtf_respawn")
				{
					if ((DateTime.Now - SpawnManager.Instance.MobileTaskForces.LastCall).TotalSeconds < 30)
					{
						ev.Success = false;
						var w = Math.Round(30 - (DateTime.Now - SpawnManager.Instance.MobileTaskForces.LastCall).TotalSeconds);
						ev.ReplyMessage = $"Последний отряд МОГ приехал менее 30 секунд назад\nПодождите {w} сек";
						return;
					}
					ev.Success = true;
					ev.ReplyMessage = "Успешно";
