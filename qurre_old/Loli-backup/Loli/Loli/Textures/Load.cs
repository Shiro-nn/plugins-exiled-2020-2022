...
namespace Loli.Textures
{
    internal static class Load
    {
        ...
        {
            new Turret(new(3.982191f, 1000, -51f), Vector3.up * 90, Color.red, 2.5f);
            ...
            try { LoadTank(); } catch { }
            try { Chaos.Load(); } catch { }
        }
        internal static SchematicUnity.API.Objects.Scheme _tank;
        private static void LoadTank()
        {
            string loc = Path.Combine(Qurre.PluginManager.PluginsDirectory, "Schemes", "Nu7Tank.json");
            _tank = SchematicUnity.API.SchematicManager.LoadSchematic(loc, new(189.84f, 992.4335f, -75.58f), Quaternion.Euler(new(0, -97)));
        }
    }
}