...
namespace Loli.Modules
{
	static class Cycle
	{
		...
		static IEnumerator<float> CountTicksUpdate()
		{
			for (; ; )
			{
				Core.TicksMinutes = Core.Ticks / 10;
				Core.Ticks = 0;
                try
				{
					if (Core.TicksMinutes > 0 && 20 >= Core.TicksMinutes)
					{
						if (LowTPS > 6 && !Round.Waiting)
						{
							new Thread(() =>
							{
								Round.End();
				...
				if (Core.ServerID != 0 && Player.List.Any())
					try { Core.Socket.Emit("server.tps", new object[] { Core.ServerID, Core.ServerName, Core.TicksMinutes }); } catch { }
