				...
				case "give":
					{
						...
						if (!(roles.Mage || roles.Sage || roles.Star || (Data.giver.TryGetValue(ev.Sender.SenderId, out var ___) && ___))) return;
						ev.Allowed = false;
						if (Round.Waiting)
						{
				...
				case "forceclass":
					{
						...
						Data.Roles.TryGetValue(ev.Sender.SenderId, out var roles);
						if (!(roles.Mage || roles.Sage || roles.Star || (Data.forcer.TryGetValue(ev.Sender.SenderId, out var ___) && ___))) return;
						ev.Allowed = false;
						if (Round.Waiting)
						{
							ev.Reply = "Раунд еще не начался";
						...
						if (team == Team.ChaosInsurgency || (role != RoleTypeId.FacilityGuard && team == Team.FoundationForces))
						{
				...
				case "pfx":
					{
						ev.Prefix = "EFFECT";
						Data.Roles.TryGetValue(ev.Sender.SenderId, out var roles);
						if (!(roles.Sage || roles.Star || (Data.effecter.TryGetValue(ev.Sender.SenderId, out var ___) && ___))) return;
						ev.Allowed = false;
						double CoolDown = 3;
				...
				case "server_event":
					{
						...
						if (ev.Args[0].ToLower() == "respawn_ci")
						{
