...
namespace Loli.Scps.Scp294
{
    static internal class Events
    {
        ...
        {
            if (pl.Inventory.UserInventory.Items.Count == 8)
            {
                pl.Hint(new(-20, 6, "<align=left><color=#F13D3D>Ваш инвентарь переполнен</color></align>", 3, false));
                return;
            }

            if (!DataBase.Manager.Static.Data.Users.TryGetValue(pl.UserId, out var data))
            {
            ...
            DataBase.Manager.Static.Stats.AddMoney(pl, -5);
