    ...
    internal static class OfflineBan
    {
        internal static void Send(RemoteAdminCommandEvent ev)
        {
            ...
            if (player != null)
            {
                ...
                Map.Broadcast($"<size=30%><color=#6f6f6f><color=#ff0000>{ev.Sender.Nickname}</color> забанил <color=#ff0000>{player.UserInfomation.Nickname}</color> " +
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        DateTime ExpireDate = DateTime.Now.AddHours(num);
                        Map.Broadcast($"<size=30%><color=#6f6f6f><color=#ff0000>{ev.Sender.Nickname}</color> забанил <color=#ff0000>{ev.Args[0]}</color> " +
