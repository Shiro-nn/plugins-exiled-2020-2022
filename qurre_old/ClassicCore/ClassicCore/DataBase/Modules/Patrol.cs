...
namespace ClassicCore.DataBase.Modules
{
	internal static class Patrol
	{
		...
		{
			ClassicCore.Init.Socket.On("database.get.patrol", obj =>
			{
			...
			static void Patrol(SendingConsoleEvent ev)
			{
				...
				new Thread(() =>
				{
					...
					listEmbed.Add(embed);
					webhk.Send("<@&722023017680732200>, вас вызывают на сервер " + ClassicCore.Init.ServerName, ClassicCore.Init.ServerName, null, false, embeds: listEmbed);
					ev.Player.SendConsoleMessage("Администрация вызвана", "white");

					new Webhook("https://discord.com/api/webhooks")
						.Send($"{ev.Player.Nickname} - {ev.Player.UserId} из патруля", ClassicCore.Init.ServerName, null, false, embeds: listEmbed);
				}).Start();
			}
		}
