...
namespace AutoEvent
{
    ...
    internal class Parkour : Event
    {
        public override string Name => "Паркурный Прикол";
        public override string Description => "";
        public override string Color => "FF4242";
        ...
        public static void OnWaitingEvent()
        {
            // Обнуление Таймера
            EventTime = new TimeSpan(0, 0, 0);
            // Создание карты
            //CreatingMapFromJson("Parkour.json", new Vector3(140f, 930f, -122.97f), out var model);
            //Model = model;
            // Создаем Чекпоинт
            CreateCheckPoint();
        ...
        public static void OnEventStarted(List<Player> players)
        {
            EventTime = new TimeSpan(0, 0, 0);
            ...
            CreatePlatformes();
            Timing.RunCoroutine(Cycle(), "jump");
        }
        ...
        public static IEnumerator<float> Cycle()
        {
            ...
            while (EventTime.TotalSeconds > 0)
            {
                ...
                Map.ClearBroadcasts();
                Map.Broadcast($"<size=50>Прыжок Веры\n" +
                    $"Пройдите до конца уровня!</size>\n" +
                    $"<size=20>Игроков: {count} | <color=red>До конца: {EventTime.Minutes}:{EventTime.Seconds} секунд</color></size>", 2);
            ...
            foreach (Player player in Player.List.ToList())
            {
                ...
                else player.Role = RoleType.Spectator;
            }
        ...
        public static void CreatePlatformes()
        {
