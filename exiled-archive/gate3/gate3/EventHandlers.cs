...
namespace gate3
{
	public partial class EventHandlers
	{
		...
		public PlayerXP.Plugin pluginpx;
		...
		public void spawn()
		{
			if (roundstart && !pluginpx.mtfvsci.GamemodeEnabled && !hasData.Gethason())
			{
		...
		private IEnumerator<float> shtimee()
		{
			for (; ; )
			{
				...
				if (spawnshtime == 0)
				{
					...
					try { discord.send.sendmsg($"<:help:671334152565424129> Приехал отряд длань змеи в кол-ве {Player.List.Where(x => x.Role == RoleType.Spectator).ToList().Count} человек."); } catch { }
		...
		*/
		...
		public void OnCheckRoundEnd(EndingRoundEventArgs ev)
		{
			...
			if(!pluginpx.mtfvsci.GamemodeEnabled && !hasData.Gethason())
			{
